...
export class CameraControls extends EventDispatcher {
	...
	constructor(
		...
		const onPointerDown = (event: PointerEvent) => {
			...
			const mouseButton =
				event.pointerType !== "mouse"
					...
					? MOUSE_BUTTON.LEFT
					: (event.buttons & MOUSE_BUTTON.MIDDLE) ===
					  MOUSE_BUTTON.MIDDLE
					? MOUSE_BUTTON.MIDDLE
					: (event.buttons & MOUSE_BUTTON.RIGHT) ===
					  MOUSE_BUTTON.RIGHT
					? MOUSE_BUTTON.RIGHT
					: null;
			...
			if (mouseButton !== null) {
				const zombiePointer =
					this._findPointerByMouseButton(mouseButton);
		...
		const onPointerMove = (event: PointerEvent) => {
			...
			const pointer =
				this._lockedPointer || this._findPointerById(pointerId);
			...
			if (event.pointerType === "touch") {
				...
				if (
					...
					(this._isDragging &&
						(event.buttons & MOUSE_BUTTON.LEFT) ===
							MOUSE_BUTTON.LEFT)
				) {
		...
		const onMouseWheel = (event: WheelEvent): void => {
			...
			const x = this.dollyToCursor
				? ((event.clientX - this._elementRect.x) /
						this._elementRect.width) *
						2 -
				  1
				: 0;
			const y = this.dollyToCursor
				? ((event.clientY - this._elementRect.y) /
		...
		const startDragging = (event?: PointerEvent): void => {
			...
			if (
				...
				(this._state & ACTION.TOUCH_ZOOM_ROTATE) ===
	 ...
	 */
	fitToBox(
		box3OrObject: _THREE.Box3 | _THREE.Object3D,
	 ...
	 */
	fitToSphere(
		sphereOrMesh: _THREE.Sphere | _THREE.Object3D,
		enableTransition: boolean
	): Promise<void[]> {
		const promises: Promise<void>[] = [];
		const isObject3D = "isObject3D" in sphereOrMesh;
		const boundingSphere = isObject3D
			? CameraControls.createBoundingSphere(
					sphereOrMesh as _THREE.Object3D,
					_sphere
			  )
	 ...
	 */
	setViewport(
		viewportOrX: _THREE.Vector4 | number | null,
		y: number,
		width: number,
		height: number
	): void {
		...
		this._viewport =
			this._viewport || (new THREE.Vector4() as _THREE.Vector4);
	...
	getPosition(
		...
		return _out
			.setFromSpherical(
				receiveEndValue ? this._sphericalEnd : this._spherical
			)
	...
	update(delta: number): boolean {
		...
		if (this.dollyToCursor) {
			if (isPerspectiveCamera(this._camera) && this._changedDolly !== 0) {
				const dollyControlAmount =
					this._spherical.radius - this._lastDistance;

				const camera = this._camera;
				const cameraDirection =
					this._getCameraDirection(_cameraDirection);
				const planeX = _v3A
				...
				const lerpRatio =
					(prevRadius - this._sphericalEnd.radius) /
					this._sphericalEnd.radius;
				const cursor = _v3C
				...
				const targetEndDiff = _v3B.subVectors(
					newTargetEnd,
					this._targetEnd
				);
	...
	protected _encloseToBoundary(
		...
		if (deltaClampedTargetLength2 === 0.0) {
			...
			const offsetFactor =
				1.0 +
				(friction * deltaClampedTargetLength2) /
					offset.dot(deltaClampedTarget);
	...
	protected _rotateInternal = (deltaX: number, deltaY: number): void => {
		const theta =
			(PI_2 * this.azimuthRotateSpeed * deltaX) /
			this._elementRect.height; // divide by *height* to refer the resolution
		const phi =
	...
	protected _dollyInternal = (delta: number, x: number, y: number): void => {
		...
		if (this.dollyToCursor) {
			this._changedDolly +=
				(this.infinityDolly ? distance : clampedDistance) -
				lastDistance;
	...
	static createBoundingSphere(
		...
		out: _THREE.Sphere = new THREE.Sphere()
	): _THREE.Sphere {
		...
		object3d.traverseVisible((object) => {
			...
			const position = bufferGeometry.attributes
				.position as _THREE.BufferAttribute;

			for (let i = 0, l = position.count; i < l; i++) {
				_v3A.fromBufferAttribute(position, i);
				maxRadiusSq = Math.max(
					maxRadiusSq,
					center.distanceToSquared(_v3A)
				);
			}
		});
